...
export default function Empilhadeiras() {
  ...
  const excluir = async (item) => {
    ...
    setConfirmar(null); setExcluindo(false); await carregar()
  }
  ...
  return (
    <div className="px-5 py-4">
      ...
      {modal && (
        <Modal title={editando ? 'Editar Operador' : 'Novo Op. Empilhadeira'} onClose={fechar}>
            ...
            <form onSubmit={salvar} className="space-y-4">
              <Field label="Nome completo" required>
                <input type="text" value={nome} onChange={e => setNome(e.target.value)}
                  required placeholder="Ex: Carlos Oliveira" className={inputClass} />
              </Field>
