  ...
  const salvar = async (e) => {
    ...
    if (editando) {
      ...
      const { error: profileErr } = await supabase.from('profiles').insert({
        ...
        perfil: 'operador', unidade_id: unidadeId, primeiro_acesso: true,
  ...
  return (
    <div className="px-5 py-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-cobeb-text font-semibold text-sm">{lista.length} operador{lista.length !== 1 ? 'es' : ''}</p>
      ...
      {loading ? (
        ...
        <p className="text-center text-slate-500 text-sm py-12">Nenhum operador encontrado</p>
      ) : (
      ...
      {modal && (
        <Modal title={editando ? 'Editar Operador' : 'Novo Operador'} onClose={fechar}>
            ...
            <form onSubmit={salvar} className="space-y-4">
              <Field label="Nome completo" required>
                <input type="text" value={nome} onChange={e => setNome(e.target.value)}
                  required placeholder="Ex: João Silva" className={inputClass} />
              </Field>
              ...
              <div>
                ...
                <p className="text-slate-400 text-xs mb-2">
                  Selecione os módulos que o operador poderá acessar. Sem seleção, verá a mensagem de nenhum módulo disponível.
                </p>
              ...
              <button type="submit" disabled={salvando}
                ...
                {salvando ? 'Salvando...' : editando ? 'Salvar alterações' : 'Criar operador'}
              </button>
            </form>
          )}
        </Modal>
  ...
  return (
    <div className="space-y-4">
      <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4">
        ...
        <p className="text-slate-500 text-xs mb-1">Login do operador:</p>
