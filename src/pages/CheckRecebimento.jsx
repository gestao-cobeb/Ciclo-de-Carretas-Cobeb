...
export default function CheckRecebimento() {
    ...
    <div className="max-w-lg mx-auto px-4 pt-3 pb-3 space-y-2 border-b border-cobeb-border/40">
      <div className="flex items-center gap-2">
        {[{ label: 'D-1', diff: -1 }, { label: 'D0', diff: 0 }, { label: 'D1', diff: 1 }].map(({ label, diff }) => {
          ...
          return (
            <button key={label} onClick={() => setFiltData(active ? '' : iso)}
              className={`${pillBase} ${active ? pillOn : pillOff} ${!hasData ? 'opacity-40' : ''}`}>
              {label}
            </button>
          )
        })}
        <input type="date" value={filtData} onChange={e => setFiltData(e.target.value)}
      ...
      <div className="relative">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar NF..."
  ...
  return (
    <AdminLayout title="Check de Recebimento" subheader={filtrosJSX}>
